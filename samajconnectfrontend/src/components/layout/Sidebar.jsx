...
export default function Sidebar({ isOpen, onClose }) {
  ...
  const { detectedCity, aqiData, requestUserLocation } = useContext(AppContext);
  ...
  return (
    <>
      ...
      <div 
        className={`fixed inset-0 bg-black/45 backdrop-blur-xs z-35 transition-opacity duration-300 lg:hidden ${
          isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
        }`}
        onClick={onClose}
      />

      <aside className={`w-[240px] fixed top-0 left-0 bottom-0 glass-elevated rounded-none border-r border-white/20 flex flex-col justify-between p-4 z-40 bg-white/45 transition-transform duration-300 ease-in-out lg:translate-x-0 ${
        ...
        <div>
          ...
          {/* Mobile-only location/AQI widgets */}
          <div className="flex flex-col gap-2 px-2 mb-4 lg:hidden">
            <div className="flex gap-2">
              <button 
                onClick={requestUserLocation}
                className="flex-1 flex items-center justify-center gap-1.5 px-2.5 py-1.5 glass border border-white/25 bg-white/10 text-primary rounded-xl text-[10px] font-bold select-none cursor-pointer"
                title="Refresh location"
              >
                <MapPin size={12} className="text-indigo-400" />
                <span>{detectedCity || "Latur"}</span>
              </button>
              
              <NavLink 
                to="/aqi" 
                onClick={onClose}
                className="flex-1 flex items-center justify-center gap-1.5 px-2.5 py-1.5 glass border border-white/25 bg-white/10 text-primary rounded-xl text-[10px] font-bold select-none"
              >
        ...
        <div className="glass p-3.5 bg-white/60 border border-white/40 rounded-2xl flex flex-col gap-2 shadow-sm">
          {loading ? (
            ...
            <div className="text-center py-2">
              <NavLink to="/login" onClick={onClose} className="text-xs font-bold text-indigo-600 hover:underline">
                Sign In to Account
              </NavLink>
            </div>
          )}
